...
本地工具下载的文件会保存到你的浏览器下载目录。下载文件、搜索历史和页面数据不会自动写入这个 GitHub 仓库，也不会修改仓库里的代码或文档文件。
...
The extension stores settings, search history, and Local Toolkit state in `chrome.storage.local`. Local Toolkit downloads are saved to the browser downloads folder; they are not written back into this GitHub repository.
